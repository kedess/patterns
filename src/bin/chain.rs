 ...
 * 
 * Применимость:
 * - когда есть более одного объекта, способного обработать запрос, причем настоящий обработчик заранее неизвестен
 *   и должен быть найден автоматически
 * - когда вы хотите отправить запрос одному из нескольких объектов, не указывая явно, какому именно.
...
impl Validator {
    fn new() -> Self {
        Validator { handler: Default::default() }
    }
}
...
impl Keeper {
    fn new() -> Self {
        Keeper { handler: Default::default() }
    }
}
...
impl Document {
    fn new() -> Self {
        Document { handler: Default::default() }
    }
